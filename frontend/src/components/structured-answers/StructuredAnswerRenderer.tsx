...
import type { Flight, StructuredPresentation } from "../../types/api";
...
function GateAssignmentCard({ data }: { data: Data }) {
  const success = text(data.status, "failed").toLowerCase() === "success";
  return <section className="rounded-2xl border border-white/10 bg-black/20 p-5"><div className="flex justify-between gap-3"><div><p className="label">Gate assignment</p><h3 className="mt-1 text-lg font-semibold text-white">{text(data.flight_number)}</h3></div><StatusBadge status={text(data.status)} kind="gate" /></div><dl className="mt-5 grid grid-cols-2 gap-4"><div><dt className="label">Previous gate</dt><dd className="mt-1 text-white">{optionalText(data.previous_gate)}</dd></div><div><dt className="label">New gate</dt><dd className="mt-1 text-white">{optionalText(data.new_gate)}</dd></div></dl><p className="mt-4 text-sm text-muted">{success ? `Assignment completed${data.terminal ? ` in ${text(data.terminal)}` : ""}.` : "The gate assignment could not be completed."}</p></section>;
}
...
export default function StructuredAnswerRenderer({ presentation, ...actions }: { presentation?: StructuredPresentation | null } & ActionProps) {
  if (!presentation?.data) return null;
  if (presentation.type === "flight_status") return <FlightStatusCard data={presentation.data} {...actions} />;
  if (presentation.type === "flight_list") return <FlightListCard data={presentation.data} {...actions} />;
  if (presentation.type === "gate_assignment") return <GateAssignmentCard data={presentation.data} />;
